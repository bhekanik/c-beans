...
# Teaching notes — Chapter 26: Navigation

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M11-1` has the planned scope.)
...
The artifact for this chapter: `j`/`k` (or arrow keys) move a highlighted cursor through the task list; `A_REVERSE` highlights the current row
