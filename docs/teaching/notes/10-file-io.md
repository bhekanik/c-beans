...
# Teaching notes — Chapter 10: File I/O

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M4-1` has the planned scope.)
...
The artifact for this chapter: A program reads a file line-by-line using `getline`, prints each line, handles EOF and missing trailing newline correctly
