...
# Teaching notes — Chapter 07: Vec_int

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M2-1` has the planned scope.)
...
The artifact for this chapter: A `Vec_int` with 100 pushes prints in order; growth policy doubles capacity; no leaks
