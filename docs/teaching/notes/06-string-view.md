...
# Teaching notes — Chapter 06: String views

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M1-2` has the planned scope.)
...
The artifact for this chapter: A function takes a `StringView` (no ownership); a builder takes a `String *` (takes ownership); the type makes the contract obvious
