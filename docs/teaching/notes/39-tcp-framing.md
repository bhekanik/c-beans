...
# Teaching notes — Chapter 39: Length-prefixed framing

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M15-2` has the planned scope.)
...
The artifact for this chapter: A 4-byte big-endian length prefix + payload framing flows correctly between client and server; partial reads are handled
