...
# Teaching notes — Chapter 09: Tagged unions

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M3-1` has the planned scope.)
...
The artifact for this chapter: A `TaskEvent` tagged union with three variants; an exhaustive switch handles every variant; the compiler complains if you add a fourth without updating the switch
