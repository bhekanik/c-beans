...
# Teaching notes — Chapter 34: Embedding sqlite

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M14-1` has the planned scope.)
...
The artifact for this chapter: sqlite3 is built via CMake `FetchContent` (amalgamation distribution); a `beans-sqlite` smoke binary opens a database, creates a `tasks` table, runs `SELECT 1`
