...
# Teaching notes — Chapter 15: DOM to typed Task

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M6-2` has the planned scope.)
...
The artifact for this chapter: `task_from_json(JsonValue *)` produces a `Task` from a JSON object; missing required fields return a typed error; unknown fields warn but don't fail
