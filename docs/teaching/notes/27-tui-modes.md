...
# Teaching notes — Chapter 27: Modes — model/update/view

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M11-2` has the planned scope.)
...
The artifact for this chapter: An `AppState`/`Msg`/`update(state, msg)` refactor lands; `NORMAL` / `INSERT` / `EDIT` modes work; the `view` function is pure
