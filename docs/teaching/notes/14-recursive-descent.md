...
# Teaching notes — Chapter 14: Recursive descent

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M6-1` has the planned scope.)
...
The artifact for this chapter: A recursive-descent parser produces a `JsonValue` DOM (tagged union) from the token stream; nested objects and arrays parse correctly; errors carry line/column
