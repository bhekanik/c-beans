...
# Teaching notes — Chapter 35: Prepared statements

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M14-2` has the planned scope.)
...
The artifact for this chapter: `sqlite_store_add` uses `sqlite3_prepare_v2` + `sqlite3_bind_text` + `sqlite3_step` + `sqlite3_finalize`; concatenated-SQL injection attempt fails harmlessly
