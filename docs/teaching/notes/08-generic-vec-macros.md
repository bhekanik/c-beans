...
# Teaching notes — Chapter 08: Generic Vec via macros

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M2-2` has the planned scope.)
...
The artifact for this chapter: `Vec_String` and `Vec_Task` (forward-declared) generated from a single `DEFINE_VEC(T)` macro
