...
# Teaching notes — Chapter 19: Atomic file writes

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M7-4` has the planned scope.)
...
The artifact for this chapter: A simulated crash mid-rewrite (Ctrl-C while `beans complete` runs) leaves the JSONL file intact; subsequent reads succeed
