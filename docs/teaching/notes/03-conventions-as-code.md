...
# Teaching notes — Chapter 03: Conventions as code

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `WS-3` has the planned scope.)
...
The artifact for this chapter: `BEANS_SANITIZERS=ON` debug build runs cleanly; clang-format check passes; CI is green
