...
# Teaching notes — Chapter 30: Wire CRUD

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M12-2` has the planned scope.)
...
The artifact for this chapter: `<Space>` toggles complete; `d` deletes (with confirmation prompt); the store updates persistently; the TUI re-renders
