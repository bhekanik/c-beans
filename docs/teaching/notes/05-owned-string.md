...
# Teaching notes — Chapter 05: Owned String

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M1-1` has the planned scope.)
...
The artifact for this chapter: A `String` struct (data + len + cap) round-trips through alloc + append + free with no leaks under ASan
