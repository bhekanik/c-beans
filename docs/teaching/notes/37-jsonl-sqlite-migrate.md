...
# Teaching notes — Chapter 37: JSONL → SQLite migration

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M14-4` has the planned scope.)
...
The artifact for this chapter: `beans migrate` reads JSONL → writes SQLite; the default config switches to SQLite; the TUI runs unchanged on the new backend
