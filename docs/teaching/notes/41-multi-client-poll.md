...
# Teaching notes — Chapter 41: Multi-client poll

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M15-4` has the planned scope.)
...
The artifact for this chapter: Two clients connect to the server simultaneously; one client's slow read doesn't block the other; `poll(2)` over the listener + each client socket
