...
# Teaching notes — Chapter 13: Number tokens

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M5-3` has the planned scope.)
...
The artifact for this chapter: Numbers parse correctly; `strtol` overflow gets reported as an error (not silently); JSON number grammar edge cases (no leading zero, optional fraction, optional exponent) covered
