...
(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M15-5` has the planned scope.)
...
The artifact for this chapter: `beans-daemon` owns the Store; `beans add` connects to the daemon and sends a Request; auto-spawn dance (probe socket → fork+exec → wait for socket) works
