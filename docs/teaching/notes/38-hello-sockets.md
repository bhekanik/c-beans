...
# Teaching notes — Chapter 38: Hello, sockets

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M15-1` has the planned scope.)
...
The artifact for this chapter: A server prints whatever a client sends; the client sends a hardcoded string; both close cleanly
