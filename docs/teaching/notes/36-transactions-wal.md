...
# Teaching notes — Chapter 36: Transactions, WAL

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M14-3` has the planned scope.)
...
The artifact for this chapter: A multi-statement bulk operation in a `BEGIN`/`COMMIT` block is all-or-nothing; WAL journal mode is enabled; a simulated crash mid-transaction leaves the db consistent
