...
# Teaching notes — Chapter 16: The Store interface

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M7-1` has the planned scope.)
...
The artifact for this chapter: A `Store` struct + `StoreVTable` with `add`/`list`/`complete`/`delete` function pointers compiles; an empty `null_store` implementation satisfies the interface
