...
# Teaching notes — Chapter 17: JSONL write

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M7-2` has the planned scope.)
...
The artifact for this chapter: `beans add "feed the cat"` opens the JSONL file in `O_APPEND`, serialises the task, writes one line; the file persists across runs
