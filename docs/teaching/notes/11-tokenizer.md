...
# Teaching notes — Chapter 11: The tokenizer

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M5-1` has the planned scope.)
...
The artifact for this chapter: A tokenizer turns `{"a": 1, "b": [true, null]}` into a stream of typed tokens with line and column positions
