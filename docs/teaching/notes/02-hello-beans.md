...
# Teaching notes — Chapter 02: Hello, beans

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `WS-2` has the planned scope.)
...
The artifact for this chapter: A `beans` binary that links `beans_core` and prints `hello, beans`
