...
# Teaching notes — Chapter 40: Protocol design

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M15-3` has the planned scope.)
...
The artifact for this chapter: A typed `Request { id, kind, payload }` and `Response { id, result, error }` (using the JSON parser from Phase B) flow over the framed connection; ids correlate request to response
