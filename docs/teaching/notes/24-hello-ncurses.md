...
# Teaching notes — Chapter 24: Hello, ncurses

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M10-1` has the planned scope.)
...
The artifact for this chapter: `beans` invoked with no args launches the TUI; it shows "hello, beans" centred, waits for a keypress, then restores the terminal cleanly
