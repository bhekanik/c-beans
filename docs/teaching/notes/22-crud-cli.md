...
# Teaching notes — Chapter 22: CRUD via CLI

> **Status:** chapter is a stub. This notes file is also a stub; both get filled when the live session runs.

## Concept anchor

(To be filled when the chapter is drafted. The session plan in `docs/teaching/sessions.md` row `M8-2` has the planned scope.)
...
(Empty — first teach hasn't happened.)

The artifact for this chapter: `beans add` / `beans list` / `beans complete <id-prefix>` / `beans delete <id-prefix>` all work end-to-end through the Store interface; `beans` with no args prints help (chapter 24 swaps that branch to launching the TUI)
...
(Empty — chapter is a stub. When the chapter ships its first verified version, capture the verification commands + outputs + environment here, mirroring chapter 01's verification record.)
