...
chapter: 28
session_id: M11-3
title: Add tasks via TUI
phase: E
estimated_time_minutes: 75
artifact: Press `a` → enter `INSERT` mode → type → press Enter → task added to store + list re-renders
status: stub
...
> Session ID: `M11-3` · Phase E · ~75 min · [Underlying milestone](../implementation/tasks/M11-tui-interactivity.md)

## What you'll learn

Line-buffer input; rendering a one-line text editor with primitives; the round-trip TUI → Store → TUI

## What you'll build

Press `a` → enter `INSERT` mode → type → press Enter → task added to store + list re-renders

## Before you start

Run chapter 27's artifact to confirm the baseline. Exact verification commands land when this chapter is drafted.
...
The companion teaching-notes file at [`docs/teaching/notes/28-tui-add.md`](../teaching/notes/28-tui-add.md) gets filled during the live session.
