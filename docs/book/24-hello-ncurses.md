...
> Session ID: `M10-1` · Phase E · ~60 min · [Underlying milestone](../implementation/tasks/M10-hello-ncurses.md)

## What you'll learn

`initscr` / `printw` / `refresh` / `getch` / `endwin`; raw mode and alternate screen; the absolute necessity of `endwin`; wiring the TUI into the no-args branch of `main`

## What you'll build

`beans` invoked with no args launches the TUI; it shows "hello, beans" centred, waits for a keypress, then restores the terminal cleanly

## Before you start

Run chapter 23's artifact to confirm the baseline. Exact verification commands land when this chapter is drafted.
...
The companion teaching-notes file at [`docs/teaching/notes/24-hello-ncurses.md`](../teaching/notes/24-hello-ncurses.md) gets filled during the live session.
