...
> Session ID: `M14-1` · Phase G · ~75 min · [Underlying milestone](../implementation/tasks/M14-sqlite.md)

## What you'll learn

sqlite as embedded library (not a server); the amalgamation distribution model; `sqlite3_open` / `sqlite3_close` lifecycle

## What you'll build

sqlite3 is built via CMake `FetchContent` (amalgamation distribution); a `beans-sqlite` smoke binary opens a database, creates a `tasks` table, runs `SELECT 1`

## Before you start

Run chapter 33's artifact to confirm the baseline. Exact verification commands land when this chapter is drafted.
...
The companion teaching-notes file at [`docs/teaching/notes/34-embed-sqlite.md`](../teaching/notes/34-embed-sqlite.md) gets filled during the live session.
