...
> Session ID: `M5-1` · Phase B · ~90 min · [Underlying milestone](../implementation/tasks/M05-tokenizer.md)

## What you'll learn

State machine, characters → tokens; separation of tokenize from parse; position tracking for error messages

## What you'll build

A tokenizer turns `{"a": 1, "b": [true, null]}` into a stream of typed tokens with line and column positions

## Before you start

Run chapter 10's artifact to confirm the baseline. Exact verification commands land when this chapter is drafted.
...
The companion teaching-notes file at [`docs/teaching/notes/11-tokenizer.md`](../teaching/notes/11-tokenizer.md) gets filled during the live session.
